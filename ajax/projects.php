...
switch ($action) {
    ...
    case 'update':
        ...
        if (!$input) jsonResponse(['success' => false, 'message' => 'No data received'], 400);
        $id = intval($input['id'] ?? 0);
        if (!$id) jsonResponse(['success' => false, 'message' => 'Missing project ID'], 400);
        ...
        try {
            ...
            foreach (['title','description','location','objectives','expected_outputs'] as $f) {
                if (array_key_exists($f, $input)) { $fields[] = "$f = ?"; $params[] = sanitize($input[$f] ?? ''); }
            }
            foreach (['program_id','funding_source_id','partner_agency_id','beneficiary_group_id'] as $f) {
                if (array_key_exists($f, $input) && $input[$f] !== '') { $fields[] = "$f = ?"; $params[] = intval($input[$f]) ?: null; }
            }
            foreach (['start_date','end_date'] as $f) {
                if (array_key_exists($f, $input)) { $fields[] = "$f = ?"; $params[] = !empty($input[$f]) ? $input[$f] : null; }
            }
            if (array_key_exists('budget', $input) && $input['budget'] !== '') { $fields[] = "budget = ?"; $params[] = floatval($input['budget']); }
            if (array_key_exists('status', $input) && $input['status'] !== '') { $fields[] = "status = ?"; $params[] = $input['status']; }
            if (array_key_exists('completion_percentage', $input) && $input['completion_percentage'] !== '') { $fields[] = "completion_percentage = ?"; $params[] = intval($input['completion_percentage']); }
            ...
            auditLog('edit', 'project', $id, 'Updated project');
