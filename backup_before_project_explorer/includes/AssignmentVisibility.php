...
class AssignmentVisibility {
    ...
    public static function getVisibleProposals($search = '', $status = '', $limit = 50, $offset = 0) {
        ...
        if ($facultyId) {
            $where .= " AND pr.project_id IN (SELECT project_id FROM project_assignments WHERE faculty_id = ? AND is_active = 1)";
            $params = array_merge([$facultyId], $params);
        }
        ...
        $sql = "SELECT pr.*, p.title as project_title, CONCAT(fp.first_name, ' ', fp.last_name) as submitter_name FROM proposals pr JOIN projects p ON pr.project_id = p.id LEFT JOIN faculty_profiles fp ON pr.submitted_by = fp.user_id{$where} ORDER BY pr.created_at DESC LIMIT ? OFFSET ?";
        $params[] = $limit; $params[] = $offset;
        $stmt = db()->prepare($sql);
        $stmt->execute($params);
